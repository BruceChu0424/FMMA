...
def write_outputs(outdir, prefix, base, code, items, symbols,
                  mem_words=isa.MEM_WORDS, source="unknown"):
    outdir = Path(outdir)
    ...
    h = [
        ...
        f"// {len(code)} words loaded at word address {base}.",
        ...
        "",
        "/* Which assembly source this image came from.  The host uses it",
        " * to report what it loaded, and to decide whether comparing the",
        " * fabric against fmma_strategy.c is meaningful - that C model",
        " * transcribes trading.asm and nothing else. */",
        f'#define FPGA_PROGRAM_NAME "{source}"',
        "",
...
def main(argv=None):
    ...
    write_outputs(args.outdir, args.prefix, args.base, code, items, symbols,
                  source=Path(args.filename).name)
